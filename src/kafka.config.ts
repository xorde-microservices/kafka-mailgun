...
const sslAuthConfig: KafkaOptions = {
  ...
  options: {
    client: {
      clientId: "client" + Math.floor(Math.random() * 10e9).toString(),
      brokers: [process.env.CONFLUENT_BOOTSTRAP], // seed broker
      ssl: true, // enable ssl
      sasl: {
    ...
    consumer: {
      groupId: "consumer" + Math.floor(Math.random() * 10e9).toString(),
    },
  },
};
